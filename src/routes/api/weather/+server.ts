...
export const GET: RequestHandler = async ({ url }) => {
  ...
  if (!city) {
    return json({ error: "City parameter is required" } as WeatherError, {
      status: 400,
    });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return json({ error: "Failed to fetch weather data" } as WeatherError, {
        status: 500,
      });
    }
    ...
    return json({ error: "An unexpected error occurred" } as WeatherError, {
      status: 500,
    });
  }
};
